...
test('bot competence gates flag worker pipeline going dark before the economy target', () => {
  const sim = new Sim({ map: sliceMap(), players: 2, seed: 8134, factions: [Terran, Terran] });
  const s = sim.fullState();
  const plan = createBotPlanner(Terran, { workerTarget: 10, barracksTarget: 1, attackThreshold: 99 })(s, 0);
  const frame = botTraceFrame(s, 0, Terran, plan);
  const darkFrames = [0, 60, 120].map((tick) => ({
    ...
    queuedWorkerProduction: 0,
    commandsByType: { ...frame.commandsByType, train: 0 },
    intentsByKind: { ...frame.intentsByKind, 'train-worker': 0 },
    objective: { ...frame.objective, queuedWorkerProduction: 0 },
    strategy: { ...frame.strategy, workerTarget: 10 },
  }));
  ...
  const failing = botTraceCompetenceGates(traceFor(darkFrames), 0).find((gate) => gate.domain === 'worker-pipeline');

  assert.equal(failing?.status, 'failing');
  assert.equal(failing?.detail.includes('below worker target without queued workers or train-worker intent'), true);

  const activeFrames = darkFrames.map((sample) => ({
    ...sample,
    queuedWorkerProduction: 1,
    intentsByKind: { ...sample.intentsByKind, 'train-worker': 1 },
    objective: { ...sample.objective, queuedWorkerProduction: 1 },
  }));
  const passing = botTraceCompetenceGates(traceFor(activeFrames), 0).find((gate) => gate.domain === 'worker-pipeline');
...
test('bot competence gates flag strategy phases without matching expert intent evidence', () => {
  const sim = new Sim({ map: sliceMap(), players: 2, seed: 8133, factions: [Terran, Terran] });
  const s = sim.fullState();
  const plan = createBotPlanner(Terran, { workerTarget: 8, barracksTarget: 1, attackThreshold: 99 })(s, 0);
  const frame = botTraceFrame(s, 0, Terran, plan);
  const frames = [0, 60, 120].map((tick) => ({
    ...frame,
    tick,
    commandsByType: { ...frame.commandsByType, build: 0, train: 0 },
    intentsByKind: {
      ...frame.intentsByKind,
      'add-production': 0,
      'spend-larva': 0,
    ...
    topIntents: [{
      ...
      status: 'done' as const,
      urgency: 24,
      axis: botIntentVictoryAxis('scout'),
      scoreReasons: [{ kind: 'map-control' as const, value: 1, detail: 'looks around instead of establishing combat' }],
      expectation: botIntentExpectation('scout'),
    }],
  }));
  const alerts = botTraceAlerts(frames);
  const stats = createMatchStats(s);
  ...
  const gate = botTraceCompetenceGates(trace, 0).find((candidate) => candidate.domain === 'plan-coherence');

  assert.equal(gate?.status, 'failing');
  assert.equal(gate?.detail.includes('establish-combat/production/rally'), true);
  assert.equal(gate?.detail.includes('production-throughput/combat-strength'), true);
});
