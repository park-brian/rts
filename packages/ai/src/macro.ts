...
import { Role, Units, eid, type Faction, type State, type Command, type Controller } from '@rts/sim';
...
export const createMacroBot = (faction: Faction): Controller => {
  ...
  return (s: State, player: number): Command[] => {
    ...
    for (let i = 0; i < e.hi; i++) {
      if (e.alive[i] !== 1 || (e.flags[i]! & Role.Producer) === 0 || e.owner[i] !== player) continue;
