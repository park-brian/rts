...
const decodedAttack = (actor: number, target: number, queue?: boolean): Extract<Command, { t: 'attack' }> => {
  const command: Extract<Command, { t: 'attack' }> = { t: 'attack', unit: actor, target };
  if (queue === true) command.queue = true;
  return command;
};
...
export const decodeAction = (action: EncodedAction): Command => {
  ...
  switch (action.head) {
    ...
    case 'attack':
      return decodedAttack(action.actor, target, action.queue);
    case 'ability': {
...
export const encodeCommand = (command: Command): EncodedAction => {
  switch (command.t) {
    ...
    case 'attack': return {
      head: 'attack', actor: command.unit, target: command.target,
      ...(command.queue === true ? { queue: true } : {}),
    };
    case 'amove': return {
...
const commandReservation = (
  ...
  switch (command.t) {
    ...
    case 'ability': {
      ...
      else if (ability.execution?.mode === 'target-spawn') r.entitySlots = ability.execution.count;
      return r;
    }
    default:
      return {};
