...
4. Replace `Game` command-option/status derivation with sim/app query modules.
   ...
   - Completed:
     ...
     - Remaining: move work-queue and smart-command derivation out of `Game` before splitting
       controllers.
...
3. Add lifecycle/status, work-queue, and selection-capability queries, then slim `Game`.
   `entityLifecycleStatus` and `selectionCapabilities` are complete; work-queue and smart-command
   queries remain.
